...
const Tracking = () => {
  const { data: deliveryData } = useGetAllParcelQuery({});
  ...
  const stats = useMemo(() => {
    ...
    const delivered = deliveryData?.data?.filter(
      (p: any) => p.status === "Delivered",
    ).length;
    const inTransit = deliveryData?.data?.filter(
      (p: any) => p.status === "In Transit",
    ).length;
    const cancelled = deliveryData?.data?.filter(
  ...
  const filteredParcels = useMemo(() => {
    if (!deliveryData?.data) return [];

    if (!searchId.trim()) return deliveryData?.data;

    return deliveryData?.data?.filter((p: any) =>
