...
const Tracking = () => {
  ...
  const { data: deliveryData } = useGetDeliveryHistoryQuery(receiverId, {
    skip: !receiverId,
  });
  ...
  const stats = useMemo(() => {
    const total = deliveryData?.data?.data?.length;
    const delivered = deliveryData?.data?.data?.filter(
      (p: any) => p.status === "Delivered",
    ).length;
    const inTransit = deliveryData?.data?.data?.filter(
      (p: any) => p.status === "In Transit",
    ).length;
    const cancelled = deliveryData?.data?.data?.filter(
  ...
  const filteredParcels = useMemo(() => {
    if (!deliveryData?.data?.data) return [];

    if (!searchId.trim()) return deliveryData.data.data;

    return deliveryData.data.data.filter((p: any) =>
